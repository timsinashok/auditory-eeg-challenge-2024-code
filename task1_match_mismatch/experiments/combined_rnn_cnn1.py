...
def combined_model(
    ...
    # CNN part for processing EEG data
    eeg_input = Input(shape=[time_window, eeg_input_dimension])
    eeg_proj_1 = Conv1D(spatial_filters, kernel_size=1)(eeg_input)
    ...
    cnn_output = Flatten()(eeg_proj_1)

    # RNN part for processing stimulus data
    stimuli_input = [Input(shape=[time_window, env_input_dimension]) for _ in range(num_mismatched_segments+1)]

    lstm_output = []
    for stimulus_input in stimuli_input:
        lstm_output.append(LSTM(lstm_units)(stimulus_input))

    # Concatenate CNN and RNN outputs
    concatenated_features = Concatenate()([cnn_output] + lstm_output)

    # Classification
    out = Dense(num_mismatched_segments + 1, activation="softmax")(concatenated_features)

    model = tf.keras.Model(inputs=[eeg_input] + stimuli_input, outputs=[out])
...
def combined_model2(
    ...
    # EEG branch
    eeg_input = tf.keras.layers.Input(shape=[time_window, eeg_input_dimension])
    eeg_conv = tf.keras.layers.Conv1D(32, kernel_size=3, activation='relu')(eeg_input)
    eeg_lstm = tf.keras.layers.LSTM(64)(eeg_conv)
    eeg_output = tf.keras.layers.Dense(64, activation='relu')(eeg_lstm)

    # Stimuli branches
    stimuli_inputs = [tf.keras.layers.Input(shape=[time_window, env_input_dimension]) for _ in range(num_mismatched_segments + 1)]
    stimuli_projections = [tf.keras.layers.Conv1D(32, kernel_size=3, activation='relu')(stimuli) for stimuli in stimuli_inputs]
    stimuli_lstm = [tf.keras.layers.LSTM(64)(stimuli_proj) for stimuli_proj in stimuli_projections]

    # Concatenate EEG and Stimuli branches
    combined = layers.concatenate([eeg_output] + stimuli_lstm)

    # Fully connected layers
    dense_1 = tf.keras.layers.Dense(128, activation='relu')(combined)
    dense_2 = tf.keras.layers.Dense(64, activation='relu')(dense_1)

    # Output layer
    output = tf.keras.layers.Dense(5, activation='softmax')(dense_2)
